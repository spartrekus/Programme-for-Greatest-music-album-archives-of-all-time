...
void drawit()
{
        ...
        erase(); 
	color_set( 0, NULL );
        mvprintw( 5 , 5 , "- Greatest Music Archive -" );

        if( playingtune == 1 )
	   mvprintw( rows-1 , 0 , "[Unix is Playing...]");

        mvprintw( 9 , 5 , "1: Dark Side of the Moon ");
	foo = 0;
  ...
  init_pair( 23, COLOR_WHITE, COLOR_BLUE);
       ...
       while( star_gameover == 0 )
       {
		...
		playingtune = 0;
	     ...
	     {
	        system( " pkill mplayer " );
	        system( " screen -d -m  mplayer -playlist playlists/DSOTM_vbr.m3u  ") ;
		playingtune = 1;
	     }
